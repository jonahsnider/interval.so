...
type Props = {
	attendee: Pick<MeetingAttendeeSchema, 'name' | 'attendanceId'>;
};
...
export function RowActionsDropdown({ attendee }: Props) {
	...
	const deleteAttendeeEntry = trpc.teams.members.deleteFinishedMeeting.useMutation({
		onMutate: () => {
			setToastId(toast.loading(`Deleting attendance record for ${attendee.name}...`));
		},
		onSuccess: () => {
			toast.success(`Deleted attendance record for ${attendee.name}`, { id: toastId });
		},
		onError: (error) => {
			toast.error(`An error occurred while deleting the attendance record for ${attendee.name}`, {
